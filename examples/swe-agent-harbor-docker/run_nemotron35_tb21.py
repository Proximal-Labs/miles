"""Nemotron 3.5 Lightning Terminal-Bench GRPO on separate nodes.

Requires two joined eight-GPU Ray nodes, matching model and dataset paths on both,
and a Harbor agent server configured for E2B and native Terminus 2. The model starts from original HF weights.

Args:
    --config: JSON object containing the ScriptArgs fields below.

Example:
    python examples/swe-agent-harbor-docker/run_nemotron35_tb21.py \
...
class ScriptArgs(U.ExecuteTrainConfig):
    ...
    data_file: str = "tb21_train.jsonl"
    megatron_path: str = "/root/Megatron-LM"
    agent_server_url: str = "http://127.0.0.1:8110"
    router_external_host: str = "127.0.0.1"
    harbor_tasks_dir: str = "/root/harbor_tasks"
    session_server_port: int = 30000
    trial_timeout: int = 14400
    ...
    wandb_project: str = "nemotron3-nano-agentic"
    ...
    def run_name(self) -> str:
        return f"{self.run_id}-nemotron35-lightning-tb21-e2b-async-2n-bs{self.global_batch_size}-g{self.group_size}"
    ...
    rollout_args = _flags(
        {
            ...
            "session_context_budget": args.context_length,
    ...
    miscellaneous_args = _flags(
        {
            ...
            "custom_megatron_before_train_step_hook_path": "tb21_training_checks.before_train_step",
