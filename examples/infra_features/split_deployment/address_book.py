...
class RunAddressBook:
    ...
    def _rpc_addr(self, *, release: str, pool_id: str) -> str:
        host = static_worker_host(release=release, component=pool_id, cell_index=0)
        port = DEFAULT_RPC_PORT_INFO.effective_static_port(worker_in_pod_index=0)
        return f"{RunNames.service_fqdn(name=host, namespace=self.namespace)}:{port}"
