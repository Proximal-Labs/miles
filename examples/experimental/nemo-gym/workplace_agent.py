...
from miles.utils.chat_template_utils import normalize_tool_arguments
...
@lru_cache(maxsize=1)
def tokenizer() -> Any:
    return AutoTokenizer.from_pretrained(os.environ["WORKPLACE_MODEL_PATH"], trust_remote_code=True)
...
async def completion(client: httpx.AsyncClient, url: str, body: dict, context: int) -> dict:
    try:
        ...
        if available >= body["max_tokens"]:
            raise
        # Rejected requests contain no generated tokens. The session server rolls
        # back its pending append before this same-transcript retry.
...
async def policy_loop(
    ...
    for _ in range(max_turns):
        # Preserve previous thinking when estimating the append-only context.
        # The margin covers parser newline drift over the bounded number of turns.
        prompt_tokens = len(
        ...
        available = min(budget - generated, context - prompt_tokens - 256)
        if available <= 0:
            stop = "response_budget" if generated >= budget else "context_limit"
            break
        body = {
            **request_kwargs,
