...
128 responses per optimizer update. The fully asynchronous producer continues
while the other node trains. It uses recorded rollout log probabilities in the
policy ratio, refreshes weights every update, and recomputes serving caches when
an in-flight request is retracted. Groups more than two published weight versions
old are regenerated; the completed-group buffer holds at most two batches.
