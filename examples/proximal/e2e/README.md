# Stage A: end-to-end from a Mac, training faked
...
Replace the stub with the platform once its endpoint registry supports the Chat Completions per-run route in [platform-contract.md](../../../docs/proximal/platform-contract.md). The launcher skips the stub whenever the run config's `platform.url` is not loopback.
