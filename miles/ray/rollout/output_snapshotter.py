...
class _RolloutExecutorOutputSnapshotter:
    ...
    def save(self, directory: Path, *, rollout_id: int) -> None:
        untrained = {
            key: (entry.data, entry.metadata) for key, entry in self._snapshots.items() if key.rollout_id > rollout_id
        }
        save_simple_checkpoint(directory=directory, data=untrained)
