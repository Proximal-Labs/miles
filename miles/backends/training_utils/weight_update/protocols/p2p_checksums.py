...
def merge_transfer_checksums(
    ...
    for shard in shards:
        if shard.cell_id not in healthy_engine_ranks:
            continue
        assert 0 <= shard.rollout_engine_rank < healthy_engine_ranks[shard.cell_id], "Unexpected P2P checksum rank"
        grouped.setdefault((shard.cell_id, shard.rollout_engine_rank), []).append(shard)
