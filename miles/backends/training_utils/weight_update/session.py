...
def check_weight_sync_results(results: list, *, is_lora: bool) -> None:
    ...
    for result in results:
        ...
        elif hasattr(result, "success"):  # config-access-exempt: weight-update transports return different result schemas
            success = result.success
            error_msg = getattr(result, "error_message", "unknown error")  # config-access-exempt: weight-update transports return different result schemas
        else:
            continue
