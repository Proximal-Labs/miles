...
class SglangConfig(FrozenStrictBaseModel):
    ...
    def parse_args(cls, args: Namespace) -> "SglangConfig":
        return cls.resolve(
            raw=_compute_raw_sglang_config(args), args=args, base_server_arg_values=_extract_base_server_arg_values(args)
        )

    def get_value(self, name: str, group: ServerGroupConfig) -> Any:
        return group.overrides.get(name, self.base_server_arg_values[name])
    ...
    @classmethod
    def resolve(
        cls, raw: _RawSglangConfig, args: Namespace, *, base_server_arg_values: dict[str, Any]
    ) -> "SglangConfig":
        ...
        return cls(models=model_configs, base_server_arg_values=base_server_arg_values)
...
def _extract_base_server_arg_values(args: Namespace) -> dict[str, Any]:
