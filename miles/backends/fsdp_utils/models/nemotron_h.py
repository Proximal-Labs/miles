...
def _patch_mixer_forward(mixer_cls):
    ...
    if getattr(orig, "_nemotron_packing", False):  # config-access-exempt: probe Nemotron patch markers and model-specific mixer state
        return

    @functools.wraps(orig)
    def cuda_kernels_forward(self, hidden_states, *args, **kwargs):
        seq_idx = getattr(self, "_packing_seq_idx", None)  # config-access-exempt: probe Nemotron patch markers and model-specific mixer state
...
def _patch_attn_forward(attn_cls):
    ...
    def forward(self, hidden_states, *args, **kwargs):
        cu = getattr(self, "_packing_cu_seqlens", None)  # config-access-exempt: probe Nemotron patch markers and model-specific mixer state
...
def apply_nemotron_h_sglang_match_patch(model):
    ...
    for mod in model.modules():
        ...
        if mixer_cls is None and cn.endswith("Mamba2Mixer") and hasattr(type(mod), "cuda_kernels_forward"):  # config-access-exempt: probe Nemotron patch markers and model-specific mixer state
            mixer_cls = type(mod)
        if attn_cls is None and getattr(mod, "block_type", None) == "attention" and hasattr(mod, "mixer"):  # config-access-exempt: probe Nemotron patch markers and model-specific mixer state
