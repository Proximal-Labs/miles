...
def apply_qwen3_dense_true_on_policy_patch(model) -> bool:
    base_model = getattr(model, "model", model)  # config-access-exempt: HF wrappers differ in base-model and final-norm layout
    final_norm = getattr(base_model, "norm", None)  # config-access-exempt: HF wrappers differ in base-model and final-norm layout
