...
class UpdateWeight(abc.ABC):
    ...
    def update_weights(self) -> None:
        ...
        bucket = []
        bucket_size = 0
        model_type = getattr(getattr(self.model, "config", None), "model_type", "")  # config-access-exempt: model wrappers and transports expose optional weight-update capabilities
        sync_dtypes = getattr(self.model, "_fsdp_sync_dtypes", None)  # config-access-exempt: model wrappers and transports expose optional weight-update capabilities
    ...
    def wait_and_update_bucket_weights(self, bucket):
        ...
        for name, param, target_dtype in bucket:
            if hasattr(param, "wait"):  # config-access-exempt: model wrappers and transports expose optional weight-update capabilities
                param = param.wait()
        ...
        if dist.get_rank() == self._ipc_gather_src:
            ...
            for i in range(num_dtypes):
                ...
                if isinstance(result, dict):
                    ...
                    success = getattr(result, "success", True)  # config-access-exempt: model wrappers and transports expose optional weight-update capabilities
                    error_msg = getattr(result, "error_message", "unknown error")  # config-access-exempt: model wrappers and transports expose optional weight-update capabilities
                if not success:
                    raise RuntimeError(
