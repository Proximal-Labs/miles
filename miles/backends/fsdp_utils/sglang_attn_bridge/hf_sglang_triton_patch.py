...
def apply_sglang_triton_attention_patch(model):
    ...
    patched = sum(1 for _, m in model.named_modules() if hasattr(m, "q_proj") and hasattr(m, "o_proj"))  # config-access-exempt: identify attention modules by their projection capabilities
    return patched
