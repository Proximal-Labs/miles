...
class FSDPTrainRayActor(TrainRayActor):
        ...
        *,
        ...
        for i in range(dist.get_world_size()):
            if i == dist.get_rank():
                ...
                if hasattr(self.hf_config, "vision_config"):  # config-access-exempt: model-family schemas differ in optional vision_config metadata
    ...
    def _get_model_cls(self):
        if hasattr(self.hf_config, "vision_config"):  # config-access-exempt: model-family schemas differ in optional vision_config metadata
            ...
            native_cls_name = MODEL_FOR_CAUSAL_LM_MAPPING_NAMES.get(getattr(self.hf_config, "model_type", ""))  # config-access-exempt: model-family schemas differ in optional model_type metadata
            if native_cls_name is not None:
                return getattr(transformers, native_cls_name)  # config-access-exempt: attribute selected at runtime from native_cls_name
