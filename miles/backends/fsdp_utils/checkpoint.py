...
def load(actor: Any) -> dict[str, Any] | None:
    ...
    load_optimizer = not actor.args.no_load_optim and hasattr(actor, "optimizer")  # config-access-exempt: optimizer is absent when the actor has no training state
    ...
    load_lr_scheduler = hasattr(actor, "lr_scheduler") and lr_scheduler_dir.exists()  # config-access-exempt: lr_scheduler is absent when the actor has no training state
    ...
    elif hasattr(actor, "lr_scheduler"):  # config-access-exempt: lr_scheduler is absent when the actor has no training state
...
def save(actor: Any, iteration: int) -> None:
    ...
    if save_optimizer_state and hasattr(actor, "optimizer") and actor.optimizer is not None:  # config-access-exempt: optimizer is absent when the actor has no training state
        optimizer_state = OptimizerState(actor.model, actor.optimizer)
        optim_state_dict = {"optim_state": optimizer_state}
        dcp.save(optim_state_dict, checkpoint_id=str(optimizer_dir))

    # Save LR scheduler state (skip if --no-save-optim is set)
    if save_optimizer_state and hasattr(actor, "lr_scheduler") and actor.lr_scheduler is not None:  # config-access-exempt: lr_scheduler is absent when the actor has no training state
        lr_scheduler_state = LRSchedulerState(actor.lr_scheduler)
        lr_scheduler_state_dict = {"lr_scheduler_state": lr_scheduler_state}
