...
def _is_qwen3(hf_config) -> bool:
    return str(getattr(hf_config, "model_type", "") or "") == "qwen3"  # config-access-exempt: model-family schemas differ in optional model_type metadata
...
def _instance_patch_applies(hf_config, args) -> bool:
    return _uses_formal_contract(hf_config, args) and not args.fp16
