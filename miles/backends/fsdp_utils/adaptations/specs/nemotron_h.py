...
def _is_nemotron_h(hf_config) -> bool:
    return str(getattr(hf_config, "model_type", "") or "") == "nemotron_h"  # config-access-exempt: model-family schemas differ in optional model_type metadata
...
def _packing_applies(hf_config) -> bool:
    return "nemotron_h" in str(getattr(hf_config, "model_type", "") or "").lower()  # config-access-exempt: model-family schemas differ in optional model_type metadata
