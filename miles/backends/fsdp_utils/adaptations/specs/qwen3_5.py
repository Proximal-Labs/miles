...
def _applies(hf_config) -> bool:
    ...
    tc = getattr(hf_config, "get_text_config", lambda: hf_config)()  # config-access-exempt: model-family schemas differ in optional get_text_config metadata
    layer_types = getattr(tc, "layer_types", None) or getattr(hf_config, "layer_types", None)  # config-access-exempt: model-family schemas differ in optional layer_types metadata
