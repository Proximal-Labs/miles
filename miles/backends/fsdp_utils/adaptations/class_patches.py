...
def check_train_infer_consistency(hf_config) -> None:
    ...
    model_type = str(getattr(hf_config, "model_type", "") or "")  # config-access-exempt: model-family schemas differ in optional model_type metadata
    is_dsa = (
        "deepseek_v3" in model_type
        or bool(getattr(hf_config, "index_topk", None))  # config-access-exempt: model-family schemas differ in optional index_topk metadata
        or getattr(hf_config, "attn_module_list_cfg", None) is not None  # config-access-exempt: model-family schemas differ in optional attn_module_list_cfg metadata
    )
    if is_dsa:
        logger.warning(
...
def check_fp8_checkpoint(hf_config) -> None:
    ...
    qc = getattr(hf_config, "quantization_config", None)  # config-access-exempt: model-family schemas differ in optional quantization_config metadata
    if not qc:
        return
    method = qc.get("quant_method") if isinstance(qc, dict) else getattr(qc, "quant_method", None)  # config-access-exempt: third-party providers differ in quant_method support
