...
def build_slot_scheduler(args: TrainerConfig, optimizer, adapter, resume_step: int) -> OptimizerParamScheduler:
    ...
    if args.trainer_backend.lr_warmup_fraction is not None and decay_steps is not None:
        lr_warmup_steps = args.trainer_backend.lr_warmup_fraction * decay_steps
    else:
        lr_warmup_steps = args.trainer_backend.lr_warmup_iters * samples_per_step
    ...
    scheduler = OptimizerParamScheduler(
        ...
        wsd_decay_steps=(
            args.trainer_backend.lr_wsd_decay_iters * samples_per_step
            if lr_decay_style == "WSD" and args.trainer_backend.lr_wsd_decay_iters is not None
            else None
        ),
