    ...
    try:
        for slot in range(args.multi_lora_n_adapters):
            ...
            children = [
                ...
                if getattr(child, "optimizer", None) is not None and child.get_parameters()  # config-access-exempt: optimizer wrappers differ in optimizer support
            ]
...
def reset_grad_metadata_keep_grads(model_chunks) -> None:
    ...
    for model_chunk in model_chunks:
        if getattr(model_chunk.config, "cuda_graph_impl", "none") != "transformer_engine":  # config-access-exempt: third-party providers differ in cuda_graph_impl support
...
def zero_adapter_slot_grads(model, slot: int) -> None:
    ...
    for param in adapter_slot_parameters(model, slot):
        if (main_grad := getattr(param, "main_grad", None)) is not None:  # config-access-exempt: main_grad is optional backend-attached tensor metadata
            main_grad.zero_()
        param.grad = None
        if (main_param := getattr(param, "main_param", None)) is not None:  # config-access-exempt: main_param is optional backend-attached tensor metadata
