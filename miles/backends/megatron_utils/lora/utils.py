...
def save_lora_checkpoint(
    ...
    *,
    ...
    save_path = artifact_store.path(save_dir)
    ...
    def prepare_checkpoint() -> None:
        if artifact_store.is_publisher:
            artifact_store.ensure_dir(save_path)

    artifact_store.run_local_phase("lora.checkpoint.prepare", prepare_checkpoint)
    ...
    artifact_store.run_local_phase(
        "lora.checkpoint.native", lambda: artifact_store.atomic_torch_save(native_path, adapter_state)
    )
    artifact_store.wait_for_all()
    ...
    try:
        ...
        if is_dp_cp_rank_0 and tp_rank == 0 and pp_rank == 0:
            artifact_store.atomic_torch_save(save_path / "adapter_model.bin", lora_state_dict)
            ...
            artifact_store.atomic_write_json(save_path / "adapter_config.json", config)
    ...
    if optimizer is not None:
        ...
        artifact_store.run_local_phase(
            "lora.checkpoint.training_state",
            lambda: artifact_store.atomic_torch_save(save_path / f"training_state_rank{rank}.pt", training_state),
        )
        artifact_store.wait_for_all()
