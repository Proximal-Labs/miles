...
def _hash_named_tensors(model: Sequence[DDP], *, accessor: str) -> dict[str, str]:
    ...
    for pp_idx, model_chunk in enumerate(model):
        for name, tensor in sorted(getattr(model_chunk, accessor)(), key=lambda x: x[0]):  # config-access-exempt: attribute selected at runtime from accessor
...
def _build_name_by_tensor_id(model: Sequence[DDP]) -> dict[_MainParamId, str]:
    ...
    for pp_idx, model_chunk in enumerate(model):
        for name, param in model_chunk.named_parameters():
            ...
            main_param = getattr(param, "main_param", None)  # config-access-exempt: main_param is optional backend-attached tensor metadata
            if main_param is None:
                assert getattr(param, "main_param_sharded", False), (  # config-access-exempt: main_param_sharded is optional backend-attached tensor metadata
...
def _iter_sub_optimizers(optimizer: MegatronOptimizer) -> Iterator[MegatronOptimizer]:
    ...
    if hasattr(optimizer, "chained_optimizers"):  # config-access-exempt: optimizer wrappers differ in chained_optimizers support
