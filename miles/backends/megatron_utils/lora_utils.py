    ...
    if marked is None:
        ...
        for chunk in model:
            for param in chunk.parameters():
                group_name = getattr(param, "_lora_grad_sum_group", None)  # config-access-exempt: _lora_grad_sum_group is optional backend-attached tensor metadata
    ...
    for group_name in ("tp", "ep"):
        ...
        for param, g_name in marked:
            if g_name != group_name:
                continue
            grad = getattr(param, "main_grad", None)  # config-access-exempt: main_grad is optional backend-attached tensor metadata
            if grad is None:
                grad = param.grad
            if grad is not None:
                grads.append(grad)
...
def is_lora_model(model: Sequence[torch.nn.Module]) -> bool:
    ...
    for model_chunk in model:
        if hasattr(model_chunk.module, "peft_config"):  # config-access-exempt: only PEFT-wrapped modules expose adapter configuration
            return True
