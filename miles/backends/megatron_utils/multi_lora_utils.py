...
def zero_optimizer_state_for_adapter(optimizer, model, idx: int) -> None:
    ...
    chained = getattr(
        optimizer, "chained_optimizers", [optimizer]
    )  # config-access-exempt: optimizer wrappers differ in chained_optimizers support
    for chained_optimizer in chained:
        inner = getattr(
            chained_optimizer, "optimizer", chained_optimizer
        )  # config-access-exempt: optimizer wrappers differ in optimizer support
        if inner is None:
            continue
