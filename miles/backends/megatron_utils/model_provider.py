...
def _apply_bridge_runtime_config(provider, args: argparse.Namespace) -> None:
    ...
    if hasattr(args, "cpu_offloading"):  # config-access-exempt: Megatron creates this flag only when CPU offload is enabled
    ...
    *,
    ...
    if args.enable_witness:
        install_witness(
            ...
            sequence_parallel=getattr(model.config, "sequence_parallel", False),  # config-access-exempt: third-party providers differ in sequence_parallel support
        )
