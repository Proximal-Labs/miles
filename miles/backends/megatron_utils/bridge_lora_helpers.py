...
def _validate_multi_lora_moe_support(args: Namespace, provider) -> None:
    ...
    if not getattr(provider, "num_moe_experts", None):  # config-access-exempt: third-party providers differ in num_moe_experts support
        return
    ...
    expert_tp = getattr(provider, "expert_tensor_parallel_size", 1) or 1  # config-access-exempt: third-party providers differ in expert_tensor_parallel_size support
    assert expert_tp == 1, (
    ...
    assert getattr(provider, "moe_grouped_gemm", False), (  # config-access-exempt: third-party providers differ in moe_grouped_gemm support
    ...
    assert not getattr(provider, "fp8", None) and not getattr(provider, "fp4", None), (  # config-access-exempt: third-party providers differ in fp8 support; third-party providers differ in fp4 support
    ...
    assert not getattr(  # config-access-exempt: third-party providers differ in moe_pad_expert_input_to_capacity support
...
def _setup_lora_model_via_bridge(args: Namespace) -> list:
    ...
    if is_multi_lora_enabled(args) and targets_expert_leaves(args.target_modules):
        ...
        if getattr(provider, "moe_permute_fusion", False):  # config-access-exempt: third-party providers differ in moe_permute_fusion support
            logger.info(
    ...
    if hasattr(provider, "dsa_attention_backend"):  # config-access-exempt: third-party providers differ in dsa_attention_backend support
    ...
    if is_value_model:
        hidden_size = hf_config.text_config.hidden_size if hasattr(hf_config, "text_config") else hf_config.hidden_size  # config-access-exempt: model-family schemas differ in optional text_config metadata
