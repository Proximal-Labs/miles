...
def quantize_params_nvfp4(args, megatron_name, converted_named_params, quantization_config):
    ...
    if args is not None and bool(getattr(args, "fp4_param", False) or getattr(args, "fp4_param_gather", False)):  # config-access-exempt: FP4 option names differ across Megatron versions
    ...
    if getattr(args, "first_last_layers_bf16", False):  # config-access-exempt: BF16 layer options are absent in older Megatron parsers
        num_layers = int(args.num_layers)
        num_layers_at_start_in_bf16 = int(getattr(args, "num_layers_at_start_in_bf16", 0))  # config-access-exempt: BF16 layer options are absent in older Megatron parsers
        num_layers_at_end_in_bf16 = int(getattr(args, "num_layers_at_end_in_bf16", 0))  # config-access-exempt: BF16 layer options are absent in older Megatron parsers
