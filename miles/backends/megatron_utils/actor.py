...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def init(
        ...
        *,
        ...
        if role != "critic":
            for m in all_replay_managers:
                m.enabled = getattr(self.args, f"use_{m.name}_replay", False)  # config-access-exempt: attribute selected at runtime from f'use_{m.name}_replay'
        ...
        self.weight_updater = WeightUpdater(
            ...
            quantization_config=getattr(self.hf_config, "quantization_config", None),  # config-access-exempt: model-family schemas differ in optional quantization_config metadata
    ...
    def _use_rollout_replay(self, m) -> bool:
        return getattr(self.args, f"use_rollout_{m.name}_replay", False)  # config-access-exempt: attribute selected at runtime from f'use_rollout_{m.name}_replay'
