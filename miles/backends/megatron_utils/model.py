...
def train_one_step(
    ...
    if (not disable_optimizer) and (not multi_lora) and (not args.check_for_nan_in_loss_and_grad):
...
def train(
    ...
    for step_id in range(num_steps_per_rollout):
        ...
        if (train_step_outcome == TrainStepOutcome.NORMAL) and is_first_replica_megatron_main_rank():
            ...
            role = getattr(model[0], "role", "actor")  # config-access-exempt: upstream model wrappers do not carry Miles actor/critic roles
