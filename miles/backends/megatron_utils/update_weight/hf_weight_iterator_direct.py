...
def _get_megatron_local_param_infos(
    ...
    for name, param in named_params_and_buffers(args, model):
        ...
        param_infos[name] = ParamInfo(
            ...
            shape=param.shape,
            attrs={
                "tensor_model_parallel": getattr(param, "tensor_model_parallel", False),  # config-access-exempt: tensor_model_parallel is optional backend-attached tensor metadata
                "partition_dim": getattr(param, "partition_dim", -1),  # config-access-exempt: partition_dim is optional backend-attached tensor metadata
                "partition_stride": getattr(param, "partition_stride", 1),  # config-access-exempt: partition_stride is optional backend-attached tensor metadata
                "parallel_mode": getattr(param, "parallel_mode", None),  # config-access-exempt: parallel_mode is optional backend-attached tensor metadata
            },
...
def all_gather_params_async(
    ...
    for info, param in param_infos_and_params:
        ...
        if "expert_bias" in info.name:
            ...
            handles.append(None)
        elif getattr(param, "parallel_mode", None) == "duplicated" or (  # config-access-exempt: parallel_mode is optional backend-attached tensor metadata
