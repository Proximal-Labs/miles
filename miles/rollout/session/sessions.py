...
def setup_session_routes(app, backend, config: SessionServerConfig, *, use_addition_r3: bool = False):
    ...
    merge_inline_system = not detect_inline_system_support(getattr(tokenizer, "chat_template", None))  # config-access-exempt: custom tokenizers may omit a chat template
