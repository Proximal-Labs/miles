...
def retarget_args(
    ...
    *,
    ...
    if engine_count is None:
        assert num_gpus_per_engine is not None and num_gpus_per_engine > 0
        engine_count = num_gpus // num_gpus_per_engine
    runtime = RolloutRuntimeState(rollout_engine_count=engine_count, rollout_gpu_count=num_gpus)
    values = dict(args) | {
        "runtime": runtime,
