...
def _introduces_replay_gap(a: Sample, b: Sample) -> bool:
    return any(getattr(a, field) is not None and getattr(b, field) is None for field in _REPLAY_FIELDS)  # config-access-exempt: attribute selected at runtime from field
