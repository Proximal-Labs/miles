...
class OpenAIEndpointTracer:
    ...
    @staticmethod
    async def create(args: Namespace, *, evaluation: bool = False) -> "OpenAIEndpointTracer":
        ...
        query = "?evaluation=true" if evaluation else ""
        response = await post(f"{session_url}/sessions{query}", {}, action="post")
