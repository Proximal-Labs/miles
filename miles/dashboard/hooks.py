...
class TrajectorySink:
    ...
    def attempt_end(self, result) -> None:
        ...
        last = samples[-1]
        status = last.status.value if getattr(last, "status", None) is not None else ""  # config-access-exempt: accept legacy samples with optional status and weight-span metadata
    ...
    def _emit(self, kind: str, sample, *, ts: float | None = None, turn: int = -1, detail: str = "") -> None:
        try:
            versions = [span.version for span in getattr(sample, "all_weight_version_spans", None) or []]  # config-access-exempt: accept legacy samples with optional status and weight-span metadata
