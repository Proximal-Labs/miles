...
def _pad_topk(topk_indices, grad_scores):
...
class _IndexerTopkScores(torch.autograd.Function):
    ...
    def backward(ctx, grad_scores):
        ...
        topk_indices, grad_scores = _pad_topk(topk_indices, grad_scores)
...
def lighting_indexer(
    q, k, weights, cu_seqlen_ks, cu_seqlen_ke, topk: int, topk_backend: str = "torch", topk_indices=None
):
    """Scores, top-k (through the indexer replay manager unless `topk_indices` is given), gathered
    scores. Returns (scores [T, topk], topk_indices [T, topk] int32)."""
