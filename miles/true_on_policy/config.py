...
def build_true_on_policy_config(args: Any) -> TrueOnPolicyConfig | None:
    if not getattr(args, "true_on_policy", False):  # config-access-exempt: launcher namespaces may not register true-on-policy options
        return None
    ...
    return TrueOnPolicyConfig(
        ...
        contract_override=getattr(args, "true_on_policy_contract", None),  # config-access-exempt: the launcher contract override is an optional extension
    )
