...
def _build_full_grad_getter(
    ...
    bucket_groups = list(getattr(model_chunk, "bucket_groups", [])) + list(  # config-access-exempt: only distributed wrappers expose gradient bucket collections
        getattr(model_chunk, "expert_parallel_bucket_groups", [])  # config-access-exempt: only distributed wrappers expose gradient bucket collections
    )
    ...
    def get_grad(param: torch.nn.Parameter) -> torch.Tensor | None:
        ...
        return param.grad if param.grad is not None else getattr(param, "main_grad", None)  # config-access-exempt: main_grad is optional backend-attached tensor metadata
...
def _log_model_grad_coverage(model: torch.nn.Module) -> None:
    ...
    for name, param in model.named_parameters():
        ...
        total += 1
        grad = param.grad if param.grad is not None else getattr(param, "main_grad", None)  # config-access-exempt: main_grad is optional backend-attached tensor metadata
        if grad is None:
            missing.append(name)
        else:
            with_grad += 1
...
def _get_phase_override_configs(args: Namespace, phase: DumperPhase) -> dict[str, Any]:
    raw = getattr(args, f"dumper_{phase.value}")  # config-access-exempt: attribute selected at runtime from f'dumper_{phase.value}'
