...
def _assert_one_source(volume: ValuesModel, sources: tuple[str, ...]) -> None:
    declared = [source for source in sources if getattr(volume, to_snake(source)) is not None]  # config-access-exempt: attribute selected at runtime from to_snake(source)
