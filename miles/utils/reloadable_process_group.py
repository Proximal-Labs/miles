...
class ReloadableProcessGroup(torch.distributed.ProcessGroup):
    ...
    def _fwd(self, method, *args, **kwargs):
        ...
        with _wrap_low_level_call():
            return getattr(inner, method)(*args, **kwargs)  # config-access-exempt: attribute selected at runtime from method
...
def _forward_remaining_collectives():
    ...
    for name in dir(dist.ProcessGroup):
        ...
        if name in vars(ReloadableProcessGroup):
            continue
        if not callable(getattr(dist.ProcessGroup, name, None)):  # config-access-exempt: attribute selected at runtime from name
            continue
