...
class InklingResponseParser:
    ...
    def __init__(self, tokenizer: Any):
        ...
        self._control_ids = frozenset({*self.ids.values(), *getattr(tokenizer, "all_special_ids", [])})  # config-access-exempt: custom tokenizers may omit special-token metadata

    def _single_token_id(self, literal: str) -> int:
        token_id = self.tokenizer.convert_tokens_to_ids(literal)
        if token_id is None or token_id == getattr(self.tokenizer, "unk_token_id", None):  # config-access-exempt: custom tokenizers may omit special-token metadata
