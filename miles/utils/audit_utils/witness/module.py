    ...
    *,
    ...
    for chunk_index, chunk in enumerate(model):
        ...
        for attr in _WITNESS_ATTRS:
            assert hasattr(inner, attr), f"chunk {chunk_index} missing {attr}"  # config-access-exempt: attribute selected at runtime from attr
            witness: _DataWitness = getattr(inner, attr)  # config-access-exempt: attribute selected at runtime from attr
...
def _has_any_witness(module: nn.Module) -> bool:
    return any(hasattr(module, attr) for attr in _WITNESS_ATTRS)  # config-access-exempt: attribute selected at runtime from attr
...
def _unwrap_to_witness_owner(chunk: nn.Module) -> nn.Module:
    ...
    inner = chunk.module
    while not _has_any_witness(inner) and hasattr(inner, "module"):  # config-access-exempt: witness metadata is attached only to instrumented modules
        inner = inner.module
    return inner
...
def _get_all_witnesses_in_model(model_chunks: Sequence[nn.Module]) -> list[_DataWitness]:
    ...
    for chunk in model_chunks:
        inner = _unwrap_to_witness_owner(chunk)
        for attr in _WITNESS_ATTRS:
            assert hasattr(inner, attr), f"model chunk missing {attr}"  # config-access-exempt: attribute selected at runtime from attr
...
def _record_and_log_witness_param(
    *,
    ...
    model_weight = witness.witness.weight
    main_param = getattr(model_weight, "main_param", None)  # config-access-exempt: witness metadata is attached only to instrumented modules
