...
def _fix_v5_tokenizer_components(tokenizer: PreTrainedTokenizerBase, model_name_or_path: str) -> None:
    ...
    backend = getattr(tokenizer, "_tokenizer", None)  # config-access-exempt: processor capabilities vary across tokenizer and modality implementations
    if backend is None:
        return
...
def processor_requires_medias(processor) -> bool:
    try:
        ...
        return hasattr(processor, "media_processor")  # config-access-exempt: processor capabilities vary across tokenizer and modality implementations
...
def process_vision_info(prompt, processor):
    if hasattr(processor, "extract_media"):  # config-access-exempt: processor capabilities vary across tokenizer and modality implementations
        return processor.extract_media(prompt)

    # TODO: temporary solution, will write image utils for miles later
    from qwen_vl_utils import process_vision_info as qwen_process_vision_info

    if hasattr(processor.image_processor, "patch_size"):  # config-access-exempt: processor capabilities vary across tokenizer and modality implementations
