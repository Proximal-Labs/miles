...
def register_hf_config_aliases() -> None:
    ...
    for alias in _CONFIG_ALIASES:
        ...
        base_config = getattr(module, alias.base_class)  # config-access-exempt: attribute selected at runtime from alias.base_class
        compat_config = type(
...
def is_dsa(hf_config) -> bool:
    return getattr(hf_config, "model_type", None) in ("deepseek_v32", "glm_moe_dsa")  # config-access-exempt: model-family schemas differ in optional model_type metadata
