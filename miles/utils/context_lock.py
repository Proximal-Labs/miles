...
def enforce_lock_discipline(cls: type) -> type:
    for member_name, member in vars(cls).items():
        ...
        for fn in _extract_checkable_functions(member):
            assert getattr(fn, _DISCIPLINE_MARKER_ATTRIBUTE_NAME, None) is not None, (  # config-access-exempt: attribute selected at runtime from _DISCIPLINE_MARKER_ATTRIBUTE_NAME
...
def _propagate_discipline_owner(fn: Callable[..., Any], owner: type) -> None:
    ...
    while target is not None and id(target) not in seen:
        ...
        if inspect.isfunction(function):
            owners = getattr(function, _DISCIPLINE_OWNER_ATTRIBUTE_NAME, None)  # config-access-exempt: attribute selected at runtime from _DISCIPLINE_OWNER_ATTRIBUTE_NAME
            if owners is None:
                setattr(function, _DISCIPLINE_OWNER_ATTRIBUTE_NAME, {owner})
            else:
                owners.add(owner)
        target = getattr(target, "__wrapped__", None)  # config-access-exempt: inspect optional decorator metadata and wrapper links


def _get_lock(obj: Any) -> ContextLock:
    lock = getattr(obj, LOCK_ATTRIBUTE_NAME)  # config-access-exempt: attribute selected at runtime from LOCK_ATTRIBUTE_NAME
...
def _assert_owner_enforces_discipline(wrapper: Callable[..., Any], obj: Any) -> None:
    owners = getattr(wrapper, _DISCIPLINE_OWNER_ATTRIBUTE_NAME, None)  # config-access-exempt: attribute selected at runtime from _DISCIPLINE_OWNER_ATTRIBUTE_NAME
