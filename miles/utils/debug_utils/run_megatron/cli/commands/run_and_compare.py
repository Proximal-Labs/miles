...
def run_and_compare(args: RunAndCompareArgs) -> None:
    ...
    common_fields: dict[str, object] = {f.name: getattr(args, f.name) for f in dataclasses.fields(CommonRunArgs)}  # config-access-exempt: attribute selected at runtime from f.name
