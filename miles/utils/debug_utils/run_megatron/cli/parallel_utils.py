...
class ParallelConfig:
    ...
    def __str__(self) -> str:
        field_str: str = ", ".join(f"{name}={getattr(self, name)}" for name in _FIELD_NAMES)  # config-access-exempt: attribute selected at runtime from name
    ...
    def dir_name(self) -> str:
        ...
        parts: list[str] = [
            ...
            if getattr(self, name) is not None and getattr(self, name) != _SKIP[name]  # config-access-exempt: attribute selected at runtime from name
        ]
        return "_".join(parts)
