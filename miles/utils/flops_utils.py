...
def calculate_fwd_flops(
    ...
    if args.num_experts is None:
        ...
        shared_expert_ffn = getattr(args, "moe_shared_expert_intermediate_size", None)  # config-access-exempt: FLOPs inputs differ between dense and shared-expert model schemas
...
def _moe_layer_pattern(config, num_layers, num_experts):
    if num_experts is None:
        return None
    first_dense = getattr(config, "first_k_dense_replace", None)  # config-access-exempt: model-family schemas differ in optional first_k_dense_replace metadata
    if first_dense is not None:
        return [0 if i < first_dense else 1 for i in range(num_layers)]
    mlp_only = set(getattr(config, "mlp_only_layers", None) or ())  # config-access-exempt: model-family schemas differ in optional mlp_only_layers metadata
    step = getattr(config, "decoder_sparse_step", 1) or 1  # config-access-exempt: model-family schemas differ in optional decoder_sparse_step metadata
    return [0 if i in mlp_only or (i + 1) % step != 0 else 1 for i in range(num_layers)]


def flops_args_from_hf_config(config):
    getter = getattr(config, "get_text_config", None)  # config-access-exempt: model-family schemas differ in optional get_text_config metadata
    config = (getter() if callable(getter) else getattr(config, "text_config", None)) or config  # config-access-exempt: model-family schemas differ in optional text_config metadata
    ...
    dense_ffn = getattr(config, "intermediate_size", None)  # config-access-exempt: model-family schemas differ in optional intermediate_size metadata

    moe_ffn = getattr(config, "moe_intermediate_size", None)  # config-access-exempt: model-family schemas differ in optional moe_intermediate_size metadata
    if num_experts is not None and moe_ffn is None:
        moe_ffn = dense_ffn

    shared_ffn = getattr(config, "shared_expert_intermediate_size", None)  # config-access-exempt: model-family schemas differ in optional shared_expert_intermediate_size metadata
    if shared_ffn is None and getattr(config, "n_shared_experts", None):  # config-access-exempt: model-family schemas differ in optional n_shared_experts metadata
    ...
    return SimpleNamespace(
        ...
        q_lora_rank=getattr(config, "q_lora_rank", None),  # config-access-exempt: model-family schemas differ in optional q_lora_rank metadata
        kv_lora_rank=getattr(config, "kv_lora_rank", None),  # config-access-exempt: model-family schemas differ in optional kv_lora_rank metadata
        qk_head_dim=getattr(config, "qk_nope_head_dim", None) or 0,  # config-access-exempt: model-family schemas differ in optional qk_nope_head_dim metadata
        qk_pos_emb_head_dim=getattr(config, "qk_rope_head_dim", None) or 0,  # config-access-exempt: model-family schemas differ in optional qk_rope_head_dim metadata
        v_head_dim=getattr(config, "v_head_dim", None) or 0,  # config-access-exempt: model-family schemas differ in optional v_head_dim metadata
    )
