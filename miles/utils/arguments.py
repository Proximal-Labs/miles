...
def _assert_reset_arg_compatible(
    *, parser: argparse.ArgumentParser, action: argparse.Action, name: str, kwargs: dict[str, Any]
    ...
    assert type(action) is expected_action, (
        ...
        def add_user_provided_function_arguments(parser):
            ...
            for path in paths:
                try:
                    ...
                    continue
                if fn is not None and callable(getattr(fn, "add_arguments", None)):  # config-access-exempt: custom hooks may optionally register CLI arguments
...
def parse_args_and_get_parser(
    ...
    if backend == "megatron":
        ...
        if args.hf_checkpoint:
            ...
            args.compress_ratios = getattr(hf_config, "compress_ratios", None)  # config-access-exempt: model-family schemas differ in optional compress_ratios metadata
            hf_validate_args(args, hf_config)

            if is_dsa(hf_config):
                args.indexer_rope_interleave = bool(getattr(hf_config, "indexer_rope_interleave", False))  # config-access-exempt: model-family schemas differ in optional indexer_rope_interleave metadata
    ...
    values = {
        ...
        if name not in backend_only_fields
    } | {
        "raw_megatron": resolve_megatron_config(
            args, base_args=backend_values if backend == "megatron" else {}
        ),
...
def _resolve_sample_ownership_check(args: argparse.Namespace) -> None:
    ...
    multi_policy = (
        ...
        and len([config for config in resolve_megatron_config(args, base_args={}).trainers if config.role == ACTOR_ROLE]) > 1
    )
    unsupported = [
        reason
...
def miles_validate_args(args):
    if args.custom_config_path:
        ...
        for k, v in data.items():
            if hasattr(args, k):  # config-access-exempt: attribute selected at runtime from k
                logger.info(f"Warning: Argument {k} is already set to {getattr(args, k)}, will override with {v}.")  # config-access-exempt: attribute selected at runtime from k
            setattr(args, k, v)
    ...
    if args.lora_rank > 0:
        ...
        if args.target_modules == "all-linear":
            ...
            hf_config = load_hf_config(args.hf_checkpoint)
            if getattr(hf_config, "kv_lora_rank", None):  # config-access-exempt: model-family schemas differ in optional kv_lora_rank metadata
                modules += ["kv_a_proj_with_mqa", "kv_b_proj"]
                if getattr(hf_config, "q_lora_rank", None):  # config-access-exempt: model-family schemas differ in optional q_lora_rank metadata
        ...
        if args.experts_shared_outer_loras and hasattr(args, "sglang_experts_shared_outer_loras"):  # config-access-exempt: older SGLang parsers omit the expert-LoRA switch
            args.sglang_experts_shared_outer_loras = True
        assert args.experts_shared_outer_loras == bool(
            getattr(args, "sglang_experts_shared_outer_loras", args.experts_shared_outer_loras)  # config-access-exempt: older SGLang parsers omit the expert-LoRA switch
...
def resolve_fsdp_num_layers(hf_config) -> int | None:
    ...
    tower instead.
    """
    getter = getattr(hf_config, "get_text_config", None)  # config-access-exempt: model-family schemas differ in optional get_text_config metadata
    text_config = (getter() if callable(getter) else getattr(hf_config, "text_config", None)) or hf_config  # config-access-exempt: model-family schemas differ in optional text_config metadata

    num_layers = getattr(text_config, "num_hidden_layers", None)  # config-access-exempt: model-family schemas differ in optional num_hidden_layers metadata
    if num_layers is None:
        num_layers = getattr(hf_config, "num_hidden_layers", None)  # config-access-exempt: model-family schemas differ in optional num_hidden_layers metadata
    return num_layers
...
def hf_validate_args(args, hf_config):
    ...
    if hasattr(hf_config, "text_config"):  # config-access-exempt: model-family schemas differ in optional text_config metadata
        hf_config = hf_config.text_config

    if hasattr(hf_config, "rope_parameters") and isinstance(hf_config.rope_parameters, dict):  # config-access-exempt: model-family schemas differ in optional rope_parameters metadata
    ...
    for hf_config_name, megatron_config_name, compare_fn in [
        ...
        if getattr(hf_config, "model_type", "") == "qwen3_5_moe_text" and hf_config_name == "intermediate_size":  # config-access-exempt: model-family schemas differ in optional model_type metadata
            continue
        if getattr(hf_config, "model_type", "") == "deepseek_v4" and hf_config_name == "intermediate_size":  # config-access-exempt: model-family schemas differ in optional model_type metadata
            continue
        if hasattr(hf_config, hf_config_name):  # config-access-exempt: attribute selected at runtime from hf_config_name
            if not compare_fn(getattr(hf_config, hf_config_name), getattr(args, megatron_config_name)):  # config-access-exempt: attribute selected at runtime from hf_config_name; attribute selected at runtime from megatron_config_name
                errors.append(
