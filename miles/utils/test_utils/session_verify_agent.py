...
def _append_session_verify_record(entry: dict) -> bool:
    ...
    fd = os.open(
        metrics_path,
        os.O_WRONLY | os.O_APPEND | os.O_CREAT | getattr(os, "O_CLOEXEC", 0),  # config-access-exempt: os.O_CLOEXEC is platform-dependent
        0o600,
    )
    try:
...
async def generate(input: GenerateFnInput) -> GenerateFnOutput:
    ...
    cycles = getattr(input.args, "session_verify_cycles", DEFAULT_CYCLES)  # config-access-exempt: optional custom session-verification hook argument
    failure_mode = getattr(input.args, "tool_call_failure_mode", DEFAULT_TOOL_CALL_FAILURE_MODE)  # config-access-exempt: optional custom session-verification hook argument
