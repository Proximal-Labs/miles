...
def dataclass_to_values(args_obj: object) -> dict[str, object]:
    return {field.name: getattr(args_obj, field.name) for field in dataclasses.fields(args_obj)}  # config-access-exempt: attribute selected at runtime from field.name
...
def _describe_mismatch(parsed: _ArgsT, wanted: _ArgsT, *, uncompared_fields: frozenset[str]) -> str:
    return ", ".join(
        ...
        if field.name not in uncompared_fields and getattr(parsed, field.name) != getattr(wanted, field.name)  # config-access-exempt: attribute selected at runtime from field.name
    )
...
def parse_declared_args(text: str, *, parser: argparse.ArgumentParser) -> dict[str, object]:
    ...
    return {dest: getattr(namespace, dest) for dest in dests}  # config-access-exempt: attribute selected at runtime from dest
