...
class DebugConfig(BaseConfig):
    ...
    enable_event_analyzer: A[
        bool,
        Arg(help="Enable event analyzer to run sanity checks (e.g. cross-replica checksum consistency) before each training step."),
    ] = False
    ...
    ci_ft_test_actions: A[
        str | None,
        Arg(
            help=(
                "JSON array of fault injection actions. Each action: "
    ...
    ci_ft_test_actions_path: A[
        str | None,
        Arg(
            help=(
                "Path of a file holding the same JSON array as --ci-ft-test-actions, read afresh every time "
    ...
    ci_inject_rollout_data_start_rollout_id: A[
        ...
        Arg(
            help=(
                "First rollout_id whose training data is replaced by the "
    ...
    env_report: A[
        ...
        Arg(
            help=(
                "Path to the json record the external launcher wrote about the launch that started "
                "this process."
            )
        ),
