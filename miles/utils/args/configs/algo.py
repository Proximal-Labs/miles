...
class AlgoConfig(BaseConfig):
    ...
    clip_grad: A[float, Arg(reset=True)] = 1.0
    calculate_per_token_loss: A[bool, Arg(reset=True)]
    lr: A[float, Arg(reset=True)] = 1e-6
    ...
    kl_coef: A[
        float,
        Arg(help="KL penalty coefficient for reward shaping. This is applied to the reward signal before advantage calculation."),
    ] = 0.00
    loss_type: A[
        str,
        Arg(
    ...
    custom_tis_function_path: A[
        str | None,
        Arg(help="Path to the custom TIS/RS function (e.g., examples/infra_features/train_infer_mismatch_helper/mis.py:compute_mis_weights_with_cp)."),
    ] = None
    custom_pg_loss_reducer_function_path: A[
        str | None,
        Arg(help="Path to a custom reducer function for pg_loss only. When set, pg_loss will use this custom reducer while other metrics (pg_clipfrac, ppo_kl, entropy_loss, etc.) still use the default sum_of_sample_mean. (e.g., examples/experimental/DrGRPO/custom_reducer.py:get_pg_loss_reducer)."),
    ] = None
