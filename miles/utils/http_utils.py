    ...
    while retry_count < max_retries:
        try:
            if action in ("delete", "get"):
                assert not payload
                response = await getattr(client, action)(url, headers=headers)  # config-access-exempt: attribute selected at runtime from action
            else:
                response = await getattr(client, action)(url, json=payload or {}, headers=headers)  # config-access-exempt: attribute selected at runtime from action
