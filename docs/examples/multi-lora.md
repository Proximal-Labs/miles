...
and every inference engine. A sampler save writes the adapter tensors and config,
then writes `META.json` after all ranks finish. Existing sampler versions cannot
be overwritten. Saving between `forward_backward` and `optim_step` neither applies
nor discards pending gradients.
...
not preserve the previous checkpoint. Existing directory and symlink checkpoints
remain loadable and can be overwritten.
