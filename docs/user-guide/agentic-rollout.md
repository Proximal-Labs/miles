...
`POST /sessions` also accepts `temperature`, `top_p`, and `top_k`. The session fills them into every chat request that omits the field, so the values reach the engine even when the agent framework does not forward `request_kwargs`; a value the request sets explicitly is kept. The agentic generator passes the sample's resolved sampling values automatically, and they never become engine launch arguments. A session created without one of the fields takes it from the run's flags: `--rollout-*` for training sessions and `--eval-*` for evaluation sessions, where an unset eval flag inherits the rollout flag. The order is therefore: a value the request sets, then a value the creation request sets, then the run's flags, then the engine's own default.
