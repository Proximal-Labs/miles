...
Use the harness's standard task group to join child agents and tools before
returning `AgentResult`. The example uses an AnyIO task group directly. Async
cleanup under AnyIO cancellation should run inside a shielded cancel scope.
External processes need an adapter-provided join boundary.
...
Opaque CLI harnesses must expose per-child context identities and join all
producers before verification observes the final task state. Existing Harbor
bindings retain their completion assumption; this example does not establish
that boundary for Claude Code.
