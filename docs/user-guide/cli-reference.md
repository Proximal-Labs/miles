...
| `--session-sample-picker-path` | `<module>.<fn>` | `keep_all` | v2 only: retains every leaf before shared-completion masking. `drop_retries` opts into sibling-order retry trimming. |
