...
| `WHEELS_TAG_X86` / `WHEELS_TAG_ARM64`                                                                  | the wheels release tags selected by `TARGETARCH`; router artifacts use their own commit-pinned release. cu13 uses the rolling `cu130-torch213-x86_64` / `cu130-torch213-aarch64` releases; cu12-x86 overrides `WHEELS_TAG_X86` with the rolling `cu129-x86_64` release                                                                                                                                                                                              |
| `SGLANG_BRANCH` / `SGLANG_COMMIT`, `MEGATRON_REPO` / `MEGATRON_BRANCH` / `MEGATRON_COMMIT`, `MILES_COMMIT` | source pins for the layered repos; empty commit args follow their branch, while release builds provide exact SHAs |


`SGL_ROUTER_COMMIT` pins both router build modes to the same source revision. The default wheel mode downloads the wheel and standalone binary for the build architecture from `sgl-router-<commit>` in `WHEELS_REPO`. `SGL_ROUTER_USE_WHEELS=0` builds that commit from `SGL_ROUTER_REPO`. Router artifacts in the rolling wheels release or local `wheels/` directory are not used.
