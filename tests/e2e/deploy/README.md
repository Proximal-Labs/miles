...
- **Weight checks**: same-version consistency and per-tensor adjacent-version movement (except LoRA or non-unit update intervals); raw P2P sender/receiver evidence must cover every publication in the uninterrupted final tail. Checks run in the soak package, never as production publication gates.
