...
- **All-target evidence**: every victim needs its own incarnation-bound state change and recovery; the same `update_id` must report the selected sender's entire pair failed while a peer target publishes. The CI entry is registered but has not been run.
...
Faults: scenario_random_crash's injection loop (shared tests/utils/soak/), with
