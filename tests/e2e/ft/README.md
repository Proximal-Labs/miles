...
- **Why `--rollout-health-check-interval 1`**: healthy generation can finish between two five-second polls; the short scenario needs at least one fresh cell reading before its injection attempt.
- **Why this scenario polls the fault window every 0.2 seconds**: colocated generation windows are only a few seconds long, so the generic two-second scheduler cadence can miss every injectable window in an eight-rollout run.
- **Why one quiescent poll on Ray only**: this scenario polls every 0.2 seconds, so a 60-poll streak spans twelve seconds instead of the ~120s the stale-status window needs, and an eight-rollout run is too short to spend on a gate that measures nothing. The Kubernetes forms (`exec_sigkill`, `delete_pod`) run at the generic two-second cadence, where the 60-poll streak really does outlast that window, so Kubernetes keeps the generic gate.
