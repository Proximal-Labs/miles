...
def test_indexer_topk_scores_backward_matches_autograd(topk):
    ...
    torch.where(topk_indices != -1, ref_scores, 0.0).sum().backward()
    ...
    torch.where(topk_indices != -1, scores, 0.0).sum().backward()

    # bf16 GEMM vs fp32 reference flips ReLU boundaries near zero, so gradients only agree loosely.
    for ref_g, tl_g in ((q_ref.grad, q_tl.grad), (k_ref.grad, k_tl.grad), (w_ref.grad, w_tl.grad)):
        assert reference.rel_diff(ref_g, tl_g) < 0.5


def test_lighting_indexer_returns_topk_of_its_own_logits():
    torch.manual_seed(0)
    q, k, weights, ks, ke = _packed_inputs([0, 512], 8, 128)
    scores, topk_indices = lighting_indexer(q, k, weights, ks, ke, topk=64)
    logits = indexer_logits(q, k, weights, ks, ke)
