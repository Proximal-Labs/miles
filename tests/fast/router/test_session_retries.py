"""Duplicate delivery shares one generation; fresh requests remain distinct."""
...
async def test_identical_prompts_with_fresh_keys_are_distinct_generations(context_core, key):
    core = context_core
    sid, _ = await _fresh_state(core)
    first = await request(core, sid, key="first")
    second = await request(core, sid, key=key)
