...
class TestOutputSnapshotReplay:
    ...
    async def test_only_untrained_sample_snapshots_are_replayed(
        ...
        if trained:
            assert restored._output_snapshotter.get(trainer_model_id=None, rollout_id=3) is None
            return
...
class TestOneDirectoryPerRolloutCheckpoint:
    async def test_the_directory_appears_only_after_every_component_saved(
        ...
        def save(directory: Path, *, rollout_id: int) -> None:
            assert not published.exists()
            original(directory, rollout_id=rollout_id)
    ...
    async def test_an_interrupted_save_publishes_nothing(
        ...
        def fail(directory: Path, *, rollout_id: int) -> None:
    ...
    async def test_a_restored_trainer_requires_the_rollout_directory(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(AssertionError, match="no rollout-side state"):
            await executor.load(5, require_complete=True)

    async def test_a_fresh_run_restores_nothing(self, tmp_path: Path) -> None:
        """A run whose trainer starts from scratch has no rollout state, and must not look for any."""
        executor = _make_executor(tmp_path, _CountingRolloutFn())
    ...
    async def test_a_rollout_directory_that_was_never_saved_starts_fresh(self, tmp_path: Path) -> None:
    ...
    async def test_a_run_that_saves_nowhere_writes_nothing(self, tmp_path: Path) -> None:
        """Without --save there is no checkpoint root, and a save must not invent one."""
        executor = _make_executor(tmp_path, _CountingRolloutFn())
