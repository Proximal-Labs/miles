...
class TestRolloutExecutorOutputSnapshotter:
    ...
    def test_only_the_newest_captures_are_retained(self, tmp_path: Path) -> None:
        ...
        snapshotter.save(tmp_path, rollout_id=-1)
    ...
    def test_restored_batches_do_not_push_out_the_newest_captures(self, tmp_path: Path) -> None:
        ...
        snapshotter.save(tmp_path, rollout_id=2)
        restored = _RolloutExecutorOutputSnapshotter(args=_args())
        restored.load(tmp_path)
    ...
    def test_an_unreplayed_restored_batch_survives_the_next_save(self, tmp_path: Path) -> None:
        ...
        restored.save(tmp_path, rollout_id=2)
    ...
    def test_a_replayed_but_untrained_batch_survives_the_next_save(self, tmp_path: Path) -> None:
        """A prefetched batch is replayed before its training step, so the next save still owes it."""
        snapshotter = _RolloutExecutorOutputSnapshotter(args=_args())
        snapshotter.capture(trainer_model_id=None, rollout_id=3, data=[Sample(index=7)], metadata={})
        snapshotter.save(tmp_path, rollout_id=2)
        restored = _RolloutExecutorOutputSnapshotter(args=_args())
    ...
    def test_a_trained_rollout_is_not_carried_into_the_next_checkpoint(self, tmp_path: Path) -> None:
        """State the trainer already consumed must not come back as an untrained batch."""
        snapshotter = _RolloutExecutorOutputSnapshotter(args=_args())
        snapshotter.capture(trainer_model_id=None, rollout_id=3, data=[Sample(index=7)], metadata={})
        snapshotter.save(tmp_path, rollout_id=3)
        restored = _RolloutExecutorOutputSnapshotter(args=_args())
        restored.load(tmp_path)
