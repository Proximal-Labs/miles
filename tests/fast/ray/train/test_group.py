...
def _rank_report(weight_version: int | None, *, updated: tuple[str, ...] = ("cell-0",)):
    return WeightUpdateReport(weight_version=weight_version, updated_cell_ids=updated)


def _assigned_info(*cell_ids: str) -> MagicMock:
    return MagicMock(engines=[SimpleNamespace(cell_id=cell_id) for cell_id in cell_ids or ("cell-0",)])
...
class TestUpdateWeightsReachesTheWorker:
    async def test_the_engine_snapshot_reaches_the_worker_and_its_version_comes_back(self):
        ...
        info = SimpleNamespace(
            snapshot_cell_id_to_hashes={"trainer-actor-0": "workers-hash-9"},
            engines=[SimpleNamespace(cell_id="cell-0")],
        )
        group = await _make_alive_controller(num_cells=1)
        for handle in get_raw_actor_handles(_cell(group, 0)):
    ...
    async def test_reloading_the_trainer_state_does_not_rewind_the_published_version(self):
        ...
        info = SimpleNamespace(snapshot_cell_id_to_hashes={}, engines=[SimpleNamespace(cell_id="cell-0")])
        group = await _make_alive_controller(num_cells=1)
