...
class WedgedBuffer(data_buffer.DataBuffer):
    ...
    async def get(self, **context) -> list[data_buffer.DataBufferInput]:
        await self._never.wait()
        raise self._producer_error or AssertionError("the wedged buffer never hands out a group")
...
class TestBatchedGet:
    ...
    async def test_a_dead_worker_fails_the_batched_wait(self, monkeypatch):
        """Waiting for a whole batch takes longer, so a dead producer has to fail the step rather than hang."""
        fn = make_fn(monkeypatch, make_args(rollout_batch_size=1), FakeDataSource())
        fn._output, _ = make_buffer()
        fn._worker = asyncio.create_task(_die_now())
        fn._worker.add_done_callback(fn._on_worker_done)
        waiting = asyncio.create_task(get_one(fn._output))
        await asyncio.sleep(0.01)

        with pytest.raises(RuntimeError, match="worker exited without an exception"):
            await asyncio.wait_for(waiting, timeout=5)

    async def test_the_worker_failure_reaches_the_buffer_without_a_scheduled_task(self, monkeypatch):
        """A notice that needed its own task could be dropped before any step ever saw the exception."""
        fn = make_fn(monkeypatch, make_args(rollout_batch_size=1), FakeDataSource())
        fn._output, _ = make_buffer()
        worker = asyncio.create_task(_die_now())
        await worker

        fn._on_worker_done(worker)

        assert isinstance(fn._output._producer_error, RuntimeError)
        with pytest.raises(RuntimeError, match="worker exited without an exception"):
            await asyncio.wait_for(get_one(fn._output), timeout=5)
