...
def _conversation():
    return [
        ...
        {"role": "assistant", "content": None, "reasoning_content": "FIRST_REASONING", "tool_calls": [{"id": "call_1", "type": "function", "function": {"name": "lookup", "arguments": '{"key":"abc"}'}}]},
...
def test_reasoning_tools_and_end_tokens_are_supervised_without_prompt_leakage():
    ...
    supervised = load_renderer().tokenizer.decode([token for token, weight in zip(tokens, mask, strict=True) if weight])
...
def test_masked_assistant_turn_is_kept_in_context():
    ...
    assert "FIRST_REASONING" not in load_renderer().tokenizer.decode([t for t, m in zip(tokens, mask, strict=True) if m])
...
def test_matches_official_renderer_with_thinking_tools_and_max_effort():
    ...
    messages = _conversation()
    tools = [{"type": "function", "function": {"name": "lookup", "description": "Lookup a key", "parameters": {"type": "object"}}}]
    ...
    expected, weights = renderer.build_supervised_example(prefix + official, TrainOnWhat.ALL_ASSISTANT_MESSAGES, effort=0.99)
