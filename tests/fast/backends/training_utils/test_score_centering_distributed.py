"""CPU/Gloo TP=2, CP=2 parity. Opt in with MILES_TEST_DISTRIBUTED=1."""
...
def _check_selected(tp: GroupInfo, dtype: torch.dtype) -> None:
    full = torch.randn(4, 8, generator=torch.Generator().manual_seed(31)).to(dtype).requires_grad_()
    local = full.detach()[:, tp.rank * 4 : (tp.rank + 1) * 4].clone().requires_grad_()
    ids = torch.tensor([[0, 3, 6, -1], [1, 4, 4, 1], [6, 3, -1, -1], [5, 2, 4, 6]])
    ...
    weights = torch.randn(4, 4, generator=torch.Generator().manual_seed(41))
...
def _check_loss(tp: GroupInfo, cp: GroupInfo, layout: str, mode: str) -> None:
    args = Namespace(
    ...
    parts = [torch.randn(total, 8, generator=generator).requires_grad_() for total in totals]
    tokens = [torch.randint(0, 7, (total,), generator=generator) for total in totals]
    masks = [torch.tensor([1, 0]), torch.tensor([1, 1, 1]), torch.empty(0)]
    advantages = [torch.randn(response, generator=generator) for response in responses]
    distributions = [torch.randn(response, 7, generator=generator).softmax(-1) for response in responses]
    ...
    batch = dict(
        ...
        rollout_topk_token_ids=[head.indices.numpy().astype("int32") for head in candidates],
        rollout_topk_log_probs=[head.values.log().numpy() for head in candidates],
        rollout_log_probs=[
