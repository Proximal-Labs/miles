...
Miles owns the inference replicas so serving cannot drift from training. The pool runs the **same Miles image** as the trainer. Its SGLang arguments are derived from the run config's `research.lora` (rank and target modules), `model_protocol` (reasoning and tool-call parsers), base model and tokenizer. They are rendered, parsed by SGLang, and checked setting by setting at deploy time, so an extra flag or alias cannot change them and a bad flag fails the deploy rather than a GPU replica. Deploy from the Miles environment, where SGLang is importable. Operational shape (GPU type, tensor parallelism, replica bounds, adapter slots, performance-only flags) lives in a separate serving config: see [serving.example.json](../../examples/proximal/serving.example.json).
